...
class barmodule:
	...
	def generateC(self, filename):
		...
		header.write("extern const int lednobyxy[{0}][{1}];\n".format(self.colsPerPanel*len(self.panels),self.rowsPerPanel))
		header.write("extern const int ledxbyno[{0}];\n".format((self.rowsPerPanel * self.colsPerPanel + self.unusedLEDsPerPanel) * len(self.panels)))
		header.write("extern const int ledybyno[{0}];\n".format((self.rowsPerPanel * self.colsPerPanel + self.unusedLEDsPerPanel) * len(self.panels)))
		...
		for x in range(self.colsPerPanel*len(self.panels)):
			for y in range(self.rowsPerPanel):
				cfile.write("{0} ".format(self.lednobyxy[y][x]))
				if y < (self.rowsPerPanel) - 1:
					cfile.write(", ")
				else:
					cfile.write("} \n")
					if x < self.colsPerPanel*len(self.panels) - 1:
						cfile.write("\t{ ")
		cfile.write("\t};\n\n")
